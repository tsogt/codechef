...
//					prevLen=Integer.parseInt(cycles[i-1].split(" ")[0]);
				...
				for(j=0;j<lenNow;j++) {
					if(lenNow==2) {
						if(adjMat[j+jj+1][(j+1)%lenNow+jj+1]!=0&&adjMat[j+jj+1][(j+1)%lenNow+jj+1]>Integer.parseInt(cycles[i].split(" ")[j+1])) {
...
//				A=obj.dijkstra(adjMat, a,b);
				obj.explored=new int[nVert+1];
				for(int z=0;z<nVert+1;z++) {
					obj.explored[z]=0;
				}
				obj.dist=new int[nVert+1];
				out.println(obj.bfs(adjMat, a, nVert+1, b));
			}
...
/*		CycledCycles obj=new CycledCycles();
		...
		int[][] adjMat=new int[nVert+1][nVert+1];
		int s=0,t=0;
		obj.bfs(adjMat, s, nVert+1,t);*/
		
		out.close();
	}
	public int bfs(int[][] adjMat, int s, int n, int t) {
...
//			System.out.println("here");
			...
			queue.remove(0);
			for(int i=0;i<n;i++) {
				if(adjMat[node][i]!=0&&explored[i]!=1) {
//					System.out.println("here");		
